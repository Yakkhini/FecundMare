...
void perf_init() {
  ...
  freq = perf_record["FecundMare"]["freq(MHz)"].value<double>().value();
  area = perf_record["FecundMare"]["area(um^2)"].value<double>().value();
...
void perf_counter_stat(core_symbol_t *cpu_symbol) {
  ...
  auto fecundmare_perf_record = perf_record.get("FecundMare")->as_table();

  fecundmare_perf_record->insert_or_assign("Total Cycle", cpu.total_cycle);
  fecundmare_perf_record->insert_or_assign("Total Instructions", cpu.iCount);
  fecundmare_perf_record->insert_or_assign("CPI", (double)cpu.total_cycle /
                                                      (double)cpu.iCount);
  fecundmare_perf_record->insert_or_assign("IPC", (double)cpu.iCount /
                                                      (double)cpu.total_cycle);
  fecundmare_perf_record->insert_or_assign("IFU", ifu_table);
  fecundmare_perf_record->insert_or_assign("IDU", idu_table);
  fecundmare_perf_record->insert_or_assign("EXU", exu_table);
  fecundmare_perf_record->insert_or_assign("LSU", lsu_table);
  fecundmare_perf_record->insert_or_assign("AXI Arbiter", axi_arbiter_table);
