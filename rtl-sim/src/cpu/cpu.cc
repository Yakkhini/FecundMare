/*
 * SPDX-FileCopyrightText: 2026 Yakkhini <Yaksiscc@gmail.com>
 *
 * SPDX-License-Identifier: MulanPSL-2.0
 */

#include <common.h>
#include <cpu/cpu.h>
#include <cpu/difftest.h>
#include <cpu/disasm.h>
#include <cpu/ftrace.h>
#include <cpu/mtrace.h>
#include <cpu/perf.h>
#include <memory/vaddr.h>
#include <sdb.h>
#include <signal.h>

#if NVBOARD
#include <nvboard.h>
#endif

#ifdef CONFIG_TARGET_ysyxSoCFull
#include <VysyxSoCFull.h>
#include <VysyxSoCFull__Dpi.h>
#include <VysyxSoCFull__Syms.h>
#endif

#ifdef CONFIG_TARGET_FecundMare
#include <VFecundMare.h>
#include <VFecundMare__Dpi.h>
#include <VFecundMare__Syms.h>
#endif

CPU cpu;
core_symbol_t *cpu_symbol;
NPCState npc_state = CORE_INIT;

static char *NPC_CHISEL = getenv("NPC_CHISEL");
static VerilatedContext *contextp;

#if CONFIG_WAVE_RECORD
static VerilatedFstC *tfp;
#endif

void finish();
void halt(int code) {
  if (npc_state == CORE_RUNNING || npc_state == CORE_PAUSE) {
    switch (code) {
    case 0:
      npc_state = CORE_HALT;
      break;
    default:
      npc_state = CORE_ABORT;
      break;
    }
  }
}

int return_status() {
  switch (npc_state) {
  case CORE_HALT:
    return 0;
  case CORE_ABORT:
    return 1;
  default:
    return 2;
  }
}

void cpu_sync();
void cpu_check();
void single_clock() {

#if CONFIG_NVBOARD
  nvboard_update();
#endif

#if CONFIG_WAVE_RECORD
  contextp->timeInc(1);
#endif
  cpu.top->clock = 1;
  cpu.top->eval();

#if CONFIG_WAVE_RECORD
  tfp->dump(contextp->time());
  contextp->timeInc(1);
#endif

  cpu.top->clock = 0;
  cpu.top->eval();

#if CONFIG_WAVE_RECORD
  tfp->dump(contextp->time());
#endif

  cpu.total_cycle++;

  cpu_sync();
  cpu_check();
}

void reset() {

#if CONFIG_WAVE_RECORD
  contextp->timeInc(1);
#endif

  cpu.top->reset = 1;

  for (int i = 0; i < 15; i++) {

#if CONFIG_NVBOARD
    nvboard_update();
#endif

#if CONFIG_WAVE_RECORD
    tfp->dump(contextp->time());
    contextp->timeInc(1);
#endif

    cpu.top->clock = 1;
    cpu.top->eval();

#if CONFIG_WAVE_RECORD
    tfp->dump(contextp->time());
    contextp->timeInc(1);
#endif

    cpu.top->clock = 0;
    cpu.top->eval();
  }

#if CONFIG_WAVE_RECORD
  tfp->dump(contextp->time());
  contextp->timeInc(1);
#endif

  cpu.top->reset = 0;
  cpu.top->clock = 1;
  cpu.top->eval();

#if CONFIG_WAVE_RECORD
  tfp->dump(contextp->time());
  contextp->timeInc(1);
#endif

  cpu.top->clock = 0;
  cpu.top->eval();

#if CONFIG_WAVE_RECORD
  tfp->dump(contextp->time());
#endif

  cpu_sync();

  cpu_check();

  Log("CORE reset done.");
}

void cpu_init(int argc, char **argv) {
#if CONFIG_DISASM
  disasm_init();
#endif

  contextp = new VerilatedContext;
  contextp->commandArgs(argc, argv);

#ifdef CONFIG_TARGET_ysyxSoCFull
  cpu.top = new VysyxSoCFull(contextp);
  cpu_symbol = cpu.top->ysyxSoCFull->asic->cpu->cpu;
#endif

#ifdef CONFIG_TARGET_FecundMare
  cpu.top = new VFecundMare(contextp);
  cpu_symbol = cpu.top->FecundMare;
#endif
  cpu.iCount = 0;
  cpu.total_cycle = 0;
  cpu.check_cycle = false;
  perf_init();
  mtrace_init();
  Log("Welcome to FecundMare Processor Core Verilating Model.");

#if CONFIG_WAVE_RECORD
  char wavefile_name[80];
  strcpy(wavefile_name, NPC_CHISEL);
  strcat(wavefile_name, "/out/waveform.fst");
  Log("Wave Path: `%s`.", wavefile_name);

  Verilated::traceEverOn(true);
  tfp = new VerilatedFstC;
  cpu.top->trace(tfp, 5);

  tfp->open(wavefile_name);
#else
  Log("Waveform Recording: " ANSI_FMT("DISABLE", ANSI_FG_GREEN) ANSI_FG_BLUE);
#endif
}

void handle_sigint(int sig) {
  if (npc_state == CORE_RUNNING && sig == SIGINT) {
    printf("\n"); // Print a newline for avoiding `^C` symbol.
    Log("CORE " ANSI_FMT("USER INTERRUPT", ANSI_FG_YELLOW) ANSI_FG_BLUE
        " at pc = 0x%08X ",
        cpu.pc);
    npc_state = CORE_PAUSE;
  }
}

void cpu_exec(int n) {
  signal(SIGINT, handle_sigint);
  switch (npc_state) {
  case CORE_INIT:
    perf_start_timer();
    reset();
    perf_end_timer();
    npc_state = CORE_RUNNING;
    break;
  case CORE_HALT:
    Log("Program already finished!");
    return;
  case CORE_ABORT:
    Log("Program already aborted!");
    return;
  default:
    break;
  }

  npc_state = CORE_RUNNING;

  switch (n) {
  case -1:
    perf_start_timer();
    while (npc_state == CORE_RUNNING) {
      single_clock();
    }
    perf_end_timer();
    break;
  default:
    perf_start_timer();
    for (int i = 0; i < n; i++) {
      if (npc_state != CORE_RUNNING) {
        break;
      }
      single_clock();
    }
    perf_end_timer();
    break;
  }

  if (npc_state == CORE_HALT || npc_state == CORE_ABORT) {
    finish();
  }
}

void cpu_sync() {
  memcpy(cpu.regs, cpu_symbol->instructionProcessing->registerFile->registers,
         sizeof(cpu.regs));

  cpu.inst = cpu_symbol->instructionDelivery->ifu->io_toIDU_bits_inst;
  cpu.check_cycle = cpu_symbol->instructionDelivery->ifu->iCount > cpu.iCount ||
                    npc_state == CORE_HALT || npc_state == CORE_ABORT;

  if (cpu.check_cycle) {
    cpu.pc_prev = cpu.pc;
    cpu.pc = cpu_symbol->instructionDelivery->ifu->diffNextPC;
    cpu.iCount = cpu_symbol->instructionDelivery->ifu->iCount;
  }

  if (npc_state == CORE_HALT) {
    // Increase PC value since simulation will halt in this cycle immediately,
    // without WB or Next IF stage.
    cpu.pc_prev = cpu.pc;
    cpu.pc += 0x4;
  }

  axi4_interface_sync(cpu_symbol);
}

void cpu_check() {

  if (cpu_symbol->instructionDelivery->idu->decodeSupport == 0) {
    Log(ANSI_FMT("ERROR INST NOT SUPPORT: ", ANSI_FG_RED) ANSI_FG_BLUE
        "DECODE " ANSI_FMT("FAILED ", ANSI_FG_RED) ANSI_FG_BLUE
        "at pc = 0x%08X",
        cpu.pc_prev);
    npc_state = CORE_ABORT;
  }

  if (cpu_symbol->instructionProcessing->haltUnit->halt) {
    halt(cpu_symbol->instructionProcessing->haltUnit->code);
  }

#if CONFIG_MTRACE || CONFIG_DIFFTEST || CONFIG_MTRACE_DB ||                    \
    CONFIG_TARGET_FecundMare
  mtrace();
#endif

  if (cpu.check_cycle == false) {
    return;
  }

#if CONFIG_DISASM
  disassembler();
#endif

#if CONFIG_FTRACE
  ftrace_check();
#endif

#if CONFIG_DIFFTEST
  if (cpu_symbol->instructionProcessing->difftestSkip) {
    difftest_skip_ref();
  } else {
    difftest_step(cpu.pc_prev, cpu.pc);
  }

  register_write_event_buffer.valid =
      cpu_symbol->instructionProcessing->registerFile->writeValid;
  if (cpu_symbol->instructionProcessing->registerFile->writeValid) {
    register_write_event_buffer.write_addr =
        cpu_symbol->instructionProcessing->registerFile
            ->io_fromProcessing_bits_writeAddr;
    register_write_event_buffer.write_data =
        cpu_symbol->instructionProcessing->registerFile
            ->io_fromProcessing_bits_writeData;
  }

  if (mmio_difftest_check()) {
    difftest_skip_ref();
  }

#endif

#if CONFIG_WATCHPOINT
  check_wp();
#endif

  cpu.check_cycle = false;

  return;
}

void finish() {
  switch (npc_state) {
  case CORE_ABORT:
    Log("CORE: " ANSI_FMT("ABORT", ANSI_FG_RED) ANSI_FG_BLUE
        " at pc = 0x%08X " ANSI_FMT("HIT BAD TRAP", ANSI_FG_RED),
        cpu.pc);
    break;
  case CORE_HALT:
    Log("CORE: " ANSI_FMT("QUIT", ANSI_FG_GREEN) ANSI_FG_BLUE
        " at pc = 0x%08X " ANSI_FMT("HIT GOOD TRAP", ANSI_FG_GREEN),
        cpu.pc);
    break;
  default:
    return;
  }

  mtrace_finish();
  perf_stat(cpu_symbol);
}

void cpu_exit() {
#if CONFIG_WAVE_RECORD
  tfp->close();
#endif

  cpu.top->final();
  delete cpu.top;

  disasm_exit();
}
